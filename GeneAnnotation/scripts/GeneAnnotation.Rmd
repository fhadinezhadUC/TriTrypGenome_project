...
Alignment Steps:\
1. tsfm_input_geneset.txt are read and their functional class are added at the end of their geneID\
2. Variable arms are removed based on reported secondary structure from genefinders\
3. gene intron is removed based in the secondary sctructure\
...
6. the result (Aligned_TriTryp_Homo.covea) is edited based on the fallowing criteria in order:\
   ...
   c)  sequences with two or more gaps next to eachotehr are removed\
7. the alignment result is saved as fasta file Aligned_TriTryp_Homo.fasta, with secondary structure saved as Aligned_TriTryp_Homo_structfile.txt\
\
\
*SplitAlignedGenes.R*
\
