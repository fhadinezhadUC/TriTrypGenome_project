...
class KLDivergence:
    """
        calculates Kullback-Leibler Divergence for two species, and provides methods for text output to be used for
    ...
    def calculate_prob_dist_pseudocounts(self, logo_dict, key):
        ...
        a stack for KLD-logo using pseudo counts.
        """
    ...
    def calculate_kld(self, logo_dict, key_back, key_fore, back_prior, fore_prior, back_post, fore_post,
                      nopseudo_post_fore):
        """
        ...
        for single in range(self.pos):
            for state in self.singles:
                ...
                if sum(state_counts_back.values()) < 7:
                ...
                # If number of background sequences is <= 5, KLD = 0
                # KLD = 0 if the number of foreground sequences, before the addition of pseudocounts, was 0
    ...
    def func_ID_KLD_2table(self, types, fore_logo_info,
                           ...
                           back_idlogo_height,
                           kld_info,
                           kld_height, functions, states, pos, fore):
        """
        ...
        for name in nameSet:
            tableDict[name] = np.zeros(len(functionlist) * pos * 4, )

        tableDict['aa'] = [item for item in functionlist for i in range(pos * 4)]
        tableDict['coord'] = [item for item in range(pos) for i in range(4)] * len(functionlist)
        tableDict['state'] = [s for s in states if not "-" in s] * len(functionlist) * pos
        ...
        for single in range(pos):
            for state in fore_logo_info[single]:
                pandasTable.loc[
                    ...
                    fore_logo_info[single][
                        state]  ## in info 2 layers?
        for single in range(pos):
