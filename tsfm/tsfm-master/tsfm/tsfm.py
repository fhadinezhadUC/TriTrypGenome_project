...
def main():
    ...
    if args.KLD or args.bt:
        ...
        # initializing class KLDivergence  ____________________________________________________________________________
        kld = MolecularInformation.KLDivergence(pos, types, pairs, basepair, single)
        results_prob_dist = {}
        post_nopseudo = {}
        for key in logo_dict.keys():
            # calculating posterior and prior probabilities ___________________________________________________________
            results_prob_dist[key] = {}
            results_prob_dist[key]['post'], results_prob_dist[key]['prior'] = kld.calculate_prob_dist_pseudocounts(
                logo_dict, key)
            post_nopseudo[key] = kld.calculate_prob_dist_nopseudocounts(logo_dict[key])
        ...
        for pair in pairwise_combinations:
            ...
            kld_info, kld_height = kld.calculate_kld(logo_dict, key_back=pair[0], key_fore=pair[1],
        ...
        if args.bt:
            ...
            for pair in pairwise_combinations:
                # pair[0] as background
                kld.func_ID_KLD_2table(types, fore_logo_info=info_height_dic[pair[1]]['info'],
